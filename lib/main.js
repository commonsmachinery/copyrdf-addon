...
var globalPageMod = pageMod.PageMod({
    include: "*",
    contentScriptFile: data.url('main_page_mod.js'),
    attachTo: ['existing', 'top'],

    onAttach: function(worker) {
        var rules;

        rules = siteRules.getRulesForSite(worker.url);
        if (rules) {
            worker.port.emit('setSiteRules', rules);
        }

        worker.port.emit('preparePage');
    }
});
...
var copyMetadata = contextMenu.Item({
    ...
    contentScriptFile: [data.url('RDFa.1.2.0.js'),
			data.url('find_metadata.js'),
			data.url('rdfxml.js'),
...
var copyPageMetadata = contextMenu.Item({
    ...
    contentScript: ('self.on("context", function(node) {' +
		    '  return pageMetadata.hasAnyMetadata(); });'),
...
var copyMainImage = contextMenu.Item({
    label: 'Copy main image with metadata',
    data: 'id-2c6ec901-d90a-4571-a02c-37fda119f4ee', // the attribute for the magic ID
