    ...
    for (i = 0; i < sites.length; i++) {
        for (j = 0; j < sites[i].urls.length; j++) {
            try {
                ...
                console.log('Error compiling site ' + sites[i].name +
...
exports.loadLocalSiteRules = function(path, notify) {
    ...
    if (path) {
        ...
        try {
            ...
            console.log('error loading local site rules: ' + err);
        ...
        if (json) {
            if (json.formatVersion === 1 &&
                ...
                console.log('not a good local site rules object: ' + text);
            ...
            onComplete: function(response) {
                ...
                if (json) {
                    if (json.formatVersion === 1 &&
                        ...
                        console.log('not a good site rules object: ' + response.text);
                        notifyText = 'Failed to update the standard site-specific rules';
                    }
                }
                else {
                    console.log('bad site rules response: ' +
